        ...
        with theme.frame("- FITNESS -"):
            ...
            with ui.tab_panels(tabs, value = 'WORKOUT DATA'):
                with ui.tab_panel('WORKOUT DATA'):
                    ...
                    lower_df= df_last8.drop(columns = [ 
                            "Frontline POW Raise", 
                            "Arnold Press", 
                            "Elbow Out Row", 
                    ...
                    upper_df=df_last8.drop(columns = [ 
                            "Stiff Legged RDL", 
                            "Hamstring Curls",
                            "Hip Thrusters",
                            "Forward Squat",
                    ...
                    def reshape_and_rename(input_df):
                        ...
                        reformed_df['Days Since Last'] = (datetime.datetime.now()  - pd.to_datetime(reformed_df['Most Recent'])).dt.days
                        return reformed_df
                    ...
                    with ui.row():
                        ui.label('Most Recent Exercises').classes("text-3xl text-bold").style('font-style: normal, font-family : "JetBrainsMono"')
                    with ui.row():
                        with ui.card():
                            ui.label("Upper Body Exercises").classes("text-xl text-bold").style('font-style: normal, font-family : "Atkinson Hyperlegible"')
                            ui.separator().classes('w-full h-1').props('color=positive')
                            ui.table(columns=[{'name' : col, 'label' : col, 'field': col} for col in upper_df.columns],rows=upper_df.to_dict('records'),).style('font-style: normal, font-family: JeBrainsMono').classes('text-lg font-normal')
                        with ui.card():
                            ui.label("Lower Body Exercises").classes("text-xl text-bold").style('font-style: normal, font-family : "Atkinson Hyperlegible"')
                            ui.separator().classes('w-full h-1').props('color=positive')
                            ui.table(columns=[{'name' : col, 'label' : col, 'field': col} for col in lower_df.columns],rows=lower_df.to_dict('records'),).style('font-style: normal, font-family: JeBrainsMono').classes('text-lg font-normal')
                        with ui.card():
                            ui.label("Abdominal Exercises").classes("text-xl text-bold").style('font-style: normal, font-family : "Atkinson Hyperlegible"')
                            ui.separator().classes('w-full h-1').props('color=positive')
                            ui.table(columns=[{'name':col, 'label':col, 'field':col} for col in abs_df.columns], rows=abs_df.to_dict('records'),).style('font-style: normal, font-family: JeBrainsMono').classes('text-lg font-normal')
                        with ui.card():
                            ui.label("Walking").classes("text-xl text-bold").style('font-style: normal, font-family : "Atkinson Hyperlegible"')
                            ui.separator().classes('w-full h-1').props('color=positive')
                            ui.table(columns=[{'name':col, 'label':col, 'field':col} for col in walk_df.columns],rows=walk_df.to_dict('records'),).style('font-style: normal, font-family: JeBrainsMono').classes('text-lg font-normal')
                    with ui.row():
                        ui.label('Cumulative Exercise Log').classes("text-3xl text-bold").style('font-style: normal, font-family : "Atkinson Hypelegible"')
                    table = ui.table(columns=[{'name' : col, 'label' : col, 'field': col} for col in df.columns],rows=df.   to_dict('records'),).style('font-style: normal, font-family: JeBrainsMono').classes('text-lg font-normal')
